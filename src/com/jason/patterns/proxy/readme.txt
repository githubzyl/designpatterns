-----静态代理-----
1、静态代理在使用时,需要定义接口或者父类,被代理对象与代理对象一起实现相同的接口或者是继承相同父类
2、需要注意的是,代理对象与目标对象要实现相同的接口,然后通过调用相同的方法来调用目标对象的方法
3、下面举个案例来解释:
...
4、静态代理总结:
   ...
   2）缺点：因为代理对象需要与目标对象实现一样的接口,所以会有很多代理类,类太多.同时,一旦接口增加方法,目标对象与代理对象都要维护.
	  思考：如何解决静态代理中的缺点呢?答案是可以使用动态代理方式。
-----动态代理-----
1、动态代理有以下特点:
   1）代理对象,不需要实现接口
...
静态代理与动态代理共同点：
	...
	思考：有时候目标对象只是一个单独的对象,并没有实现任何的接口，如何实现代理？答案就是Cglib代理。
-----Cglib代理-----
1、Cglib代理,也叫作子类代理，它是在内存中构建一个子类对象从而实现对目标对象功能的扩展。
...
2、Cglib子类代理实现方法：
   1）需要引入cglib的jar文件,但是Spring的核心包中已经包括了Cglib功能,所以直接引入spring-core-3.2.0.jar即可.
